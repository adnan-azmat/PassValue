...
def plot_frame( hometeam, awayteam, figax=None, team_colors=('r','b'), field_dimen = (106.0,68.0), include_player_velocities=False, PlayerMarkerSize=10, PlayerAlpha=0.7, annotate=False ):
    ...
    for team,color in zip( [hometeam,awayteam], team_colors) :
        ...
        ax.plot( team[x_columns], team[y_columns], color+'o', ms=PlayerMarkerSize, alpha=PlayerAlpha ) # plot player positions
    ...
    # plot ball
    ax.plot( hometeam['ball_x'], hometeam['ball_y'], 'ko', ms=6, alpha=1.0, lw=0)
    return fig,ax
    ...
    with writer.saving(fig, fname, 100):
        for i in index:
            ...
            for team,color in zip( [hometeam.loc[i],awayteam.loc[i]], team_colors) :
                x_columns = [c for c in team.keys() if c[-2:].lower()=='_x' and c!='ball_x'] # column header for player x positions
                y_columns = [c for c in team.keys() if c[-2:].lower()=='_y' and c!='ball_y'] # column header for player y positions
                objs, = ax.plot( team[x_columns], team[y_columns], color+'o', ms=PlayerMarkerSize, alpha=PlayerAlpha ) # plot player positions
                figobjs.append(objs)
                if include_player_velocities:
                    vx_columns = ['{}_vx'.format(c[:-2]) for c in x_columns] # column header for player x positions
                    vy_columns = ['{}_vy'.format(c[:-2]) for c in y_columns] # column header for player y positions
                    ...
                    figobjs.append(objs)
            # plot ball
            objs, = ax.plot( team['ball_x'], team['ball_y'], 'ko', ms=6, alpha=1.0, lw=0)
            figobjs.append(objs)
